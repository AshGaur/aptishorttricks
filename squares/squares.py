...
start  = all_ranges.get('squares').get('start')
end    = all_ranges.get('squares').get('end')

lst = []
cur_data = {}

def practice():
	lst,cur_data = ranger.rangemaker("squ",start,end)
	accu_data = {}
	try:
	...
	try:
		...
		ranger.write_range("squ",lst,cur_data)
	except:
		if inp=="exit":
			sys.exit()
		else:
